...
なぜ制御ノードから分離したか:
  ...
  - 制御ノードが落ちても止まる。停止指令を出す主体が制御ノード自身だと、
    それが死んだ瞬間に誰も止められなくなる(AckermannSteering は指令を
    ラッチするので走り続ける)。
...
(Nav2 の nav2_collision_monitor と同じ形)
    ...
    python3 /nodes/safety_node.py --ros-args --params-file /config/safety_params.yaml
"""
